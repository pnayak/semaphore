...
function playTheBookOLD (task, playbook, done) {
...
function playTheBook (task, playbook, done) {
	...
	var devOpsHome = '/root/devops/intellify'
	...
	args.push('dev1');
	args.push('foobar');
	args.push('allCollectorTests');

	var playbook = spawn("/root/devops/intellify/scripts/tests/test.sh", args, {
		cwd: devOpsHome,
		env: {
			HOME: devOpsHome,
			OLDPWD: home,
			...
			LANG: 'en_GB.UTF-8',
			PYTHONUNBUFFERED: 1,
			ANSIBLE_HOSTS: './inventory/dev1/hosts.yml',
